...
def mfcc_auth(data_matrix, label_matrix, predict_matrix):
    ...
    rbf_kernel_svm_clf.fit(data_matrix, label_matrix)
    result = rbf_kernel_svm_clf.predict(predict_matrix).tolist()
    print(result)
    counter = 0
    for each in result:
        if each == 1:
    ...
    print(counter, len(result), percentage)
    if percentage >= 0.98:
        return True
    else:
        return False
...
def load_from_other_csvs_refreshed(file_dir, current_filename, tag):
    ...
    all_files = sample(all_files, 16)  # 随机提取16个文件名
